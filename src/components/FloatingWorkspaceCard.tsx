...
import { Minus, Plus, ArrowUpDown, ArrowLeftRight } from 'lucide-react';
...
export const FloatingWorkspaceCard: React.FC<FloatingWorkspaceCardProps> = ({
  ...
  return (
    <div
      className="absolute top-4 right-4 sm:top-6 sm:right-6 z-30 flex flex-col gap-2 p-2 bg-slate-900/90 hover:bg-slate-900/95 border border-slate-800/90 rounded-2xl backdrop-blur-xl shadow-2xl select-none transition-all duration-200 w-[240px]"
    >
      {/* FILA 1: Los 4 tamaños de aspecto en una sola fila */}
      <div className="grid grid-cols-4 gap-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80 text-xs font-mono w-full">
        {ratios.map((item) => (
          <button
            key={item.id}
            type="button"
      ...
      {/* FILA 2: Controles de Zoom en su propia fila */}
      <div className="flex items-center justify-between gap-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80 text-xs font-mono w-full">
        <button
          ...
          title="Alejar (-10%)"
          className="w-7 h-7 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white flex items-center justify-center transition"
        >
        ...
        <button
          ...
          className="flex-1 py-1 rounded-lg text-indigo-300 font-bold hover:text-white text-xs transition text-center"
        >
          {pct}%
        </button>
      ...
      {/* FILA 3: Alto, Ancho y 100% en una sola fila */}
      <div className="grid grid-cols-3 gap-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80 text-xs font-mono w-full">
        <button
          type="button"
